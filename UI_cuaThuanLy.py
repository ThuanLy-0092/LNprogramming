...
if __name__ == "__main__":
    ...
    st.caption(
        "- Nhập hệ số đi kèm với biến xuất hiện trong hàm mục tiêu ( với hệ số dương ta chỉ cần nhập **số nguyên**, còn hệ số âm ta nhập thêm dấu **-**).",
...
1 -1 0 -1 = 5""")
    ...
    LNProgramming = LinearProgrammingSolver(objective=objective_converted, lhs=lhs, rhs=rhs, bounds=bounds_convert,
                                             num_var=int(soBien), opt=opt)
    ...
    st.write("  Số bước lặp:", solution.nit) 
